...
// naval.io: Submarine + Torpedo unit type strings
// Reuse existing atlas columns for now (sub=warship, torpedo=shell)
const UT_SUBMARINE = "Submarine" as const;
const UT_TORPEDO = "Torpedo" as const;
...
const MISSILE_TYPES: ReadonlySet<string> = new Set([
  ...
  UT_SHELL,
  UT_MIRV_WARHEAD,
  // naval.io: Torpedoes are projectiles, render in missile layer
  UT_TORPEDO,
]);
...
export class UnitPass {
  ...
  updateUnits(units: Map<number, UnitState>, tick: number): void {
    ...
    for (const unit of units.values()) {
      ...
      // naval.io: Submarines reuse Warship atlas column, Torpedoes reuse Shell column
      if (atlasIdx === undefined && unit.unitType === UT_SUBMARINE) {
        atlasIdx = this.typeToAtlasCol.get(UT_WARSHIP);
      }
      if (atlasIdx === undefined && unit.unitType === UT_TORPEDO) {
        atlasIdx = this.typeToAtlasCol.get(UT_SHELL);
      }
      ...
      if (isMissile) {
        ...
        // naval.io: Torpedoes also emit a trail instance
        if (unit.unitType === UT_TORPEDO && unit.lastPos !== unit.pos) {
          const lx = unit.lastPos % this.mapW;
          const ly = (unit.lastPos - lx) / this.mapW;
          this.emitMissile(lx, ly, unit.ownerID, atlasIdx, flags);
        }
