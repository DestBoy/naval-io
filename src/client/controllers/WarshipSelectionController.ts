...
export class WarshipSelectionController implements Controller {
  ...
  private findWarshipsNearCell(clickRef: TileRef): UnitView[] {
    const myPlayer = this.game.myPlayer();
    if (!myPlayer) return [];
    // naval.io: include submarines in selection
    return [
      ...this.game.units(UnitType.Warship),
      ...this.game.units(UnitType.Submarine),
    ]
      .filter(
        (unit) =>
          unit.isActive() &&
  ...
  private onSelectionBoxComplete(event: WarshipSelectionBoxCompleteEvent) {
    ...
    // naval.io: box-select includes submarines
    const selected = [
      ...this.game.units(UnitType.Warship),
      ...this.game.units(UnitType.Submarine),
    ].filter((unit) => {
  ...
  private onSelectAllWarships() {
    ...
    // naval.io: select-all includes submarines
    const allWarships = [
      ...this.game.units(UnitType.Warship),
      ...this.game.units(UnitType.Submarine),
    ].filter((u) => u.isActive() && u.owner() === myPlayer);
