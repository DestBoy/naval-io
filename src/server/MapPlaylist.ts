...
// naval.io: Maps that don't fit the naval theme are excluded from rotation.
const NAVAL_IO_DISABLED_MAPS = new Set<GameMapType>([
  GameMapType.BaikalNukeWars,
]);

// naval.io: ONLY the most water-dominated maps. Every map here is mostly
// ocean with small islands. Future: procedural island generator will
// replace this entirely.
const NAVAL_IO_WHITELISTED_MAPS = new Set<GameMapType>([
  GameMapType.ArchipelagoSea,
  GameMapType.WarshipWarship,
...
export class MapPlaylist {
  ...
  private buildMapsList(type: PublicGameType): GameMapType[] {
    ...
    allMaps.forEach((mapInfo) => {
      ...
      // naval.io: skip maps that don't fit the naval theme
      if (NAVAL_IO_DISABLED_MAPS.has(map)) {
        return;
      }
      // naval.io: only include whitelisted naval-themed maps
      if (!NAVAL_IO_WHITELISTED_MAPS.has(map)) {
        return;
      }
