...
export class NationWarshipBehavior {
  ...
  // naval.io: AI logic for spawning submarines
  maybeSpawnSubmarine(): boolean {
    if (this.player === null) throw new Error("not initialized");
    if (this.game.config().isUnitDisabled(UnitType.Submarine)) {
      return false;
    }
    if (!this.random.chance(200)) {
      return false;
    }
    const ports = this.player.units(UnitType.Port);
    if (ports.length === 0) return false;
    const subs = this.player.units(UnitType.Submarine);
    if (subs.length >= 3) return false;
    const cost = this.cost(UnitType.Submarine);
    if (this.player.gold() <= cost) return false;
    const port = this.random.randElement(ports);
    const targetTile = this.warshipSpawnTile(port.tile(), 250);
    if (targetTile === null) return false;
    const canBuild = this.player.canBuild(UnitType.Submarine, targetTile);
    if (canBuild === false) return false;
    this.game.addExecution(
      new ConstructionExecution(this.player, UnitType.Submarine, targetTile),
    );
    return true;
  }
