...
export class Config {
  ...
  isUnitDisabled(unitType: UnitType): boolean {
    // naval.io: Modern warfare units and trains are permanently disabled.
    // The underlying code still exists for now (will be removed in a later
    // refactor), but neither players nor AI can build these units.
    const NAVAL_IO_DISABLED_UNITS: ReadonlySet<UnitType> = new Set([
      UnitType.AtomBomb,
    ...
    if (NAVAL_IO_DISABLED_UNITS.has(unitType)) {
      return true;
    }
  ...
  unitInfo(type: UnitType): UnitInfo {
    ...
    switch (type) {
      ...
      case UnitType.Submarine:
        ...
        break;
      // naval.io: Torpedo (projectile, free to spawn)
