...
export class UserSettings {
  ...
  keybinds(isMac: boolean): Record<string, string> {
    ...
    const userKeybinds = this.normalizedUserKeybinds();

    // naval.io: settings migration. When defaults change, force-overwrite
    // saved user keybinds so they pick up the new layout.
    const NAVAL_IO_KEYBINDS_VERSION = "2";
    const storedVersion = this.getString("settings.keybindsVersion", "");
    if (storedVersion !== NAVAL_IO_KEYBINDS_VERSION) {
      this.setString(KEYBINDS_KEY, JSON.stringify(defaults));
      this.setString("settings.keybindsVersion", NAVAL_IO_KEYBINDS_VERSION);
